...
const API_BASE_URL = 'http://192.168.1.2:8083/api/report';


export const generateReport = async (reportData) => {
    try {
        // Realiza la solicitud POST y espera la respuesta con el archivo PDF
        const response = await axios.post(`${API_BASE_URL}/generateReport`, reportData, {
        ...
        const reportType = type === 'income' ? 'Ingresos' : (type === 'savings' ? 'Ahorros' : (type === 'expense' ? 'Egresos' : 'Todos'));
