...
const API_BASE_URL = 'http://192.168.1.2:8084/api/settings';
...
export const getUserFavoriteColors = async () => {
    if (token) {
        try {
            const response = await axios.get(`${API_BASE_URL}/viewUserFavoriteColors`, {
                headers: {
...
export const updateUserFavoriteColors = async (colorsData) => {
    if (token) {
        try {
            const response = await axios.put(`${API_BASE_URL}/updateFavoriteColors`, colorsData, {
                headers: {
                    'Authorization': `Bearer ${token}` 
                }
            });
...
export const resetToDefaultColors = async () => {
    if (token) {
        try {
            const response = await axios.put(`${API_BASE_URL}/resetToDefaultColors`,{} ,{
                headers: {
                    'Authorization': `Bearer ${token}` 
                }
            });
...
export const updateUserSettings = async (settingsData) => {
    if (token) {
        try {
            const response = await axios.put(`${API_BASE_URL}/updateUserSettings`, settingsData, {
                headers: {
                    'Authorization': `Bearer ${token}` 
                }
            });
...
export const resetToDefaultSettings = async () => {
    if (token) {
        try {
            const response = await axios.put(`${API_BASE_URL}/resetToDefaultSettings`,{} ,{
                headers: {
                    'Authorization': `Bearer ${token}` 
                }
            });
